...
exports.getChildData = async (req, res) => {
  try {
    ...
    if (childId) {
      existingChild = await childModel.findById(childId).select('_id user_id code gender points birth_date').lean();
    }
    if (!existingChild && userId) {
      existingChild = await childModel.findOne({ user_id: userId }).select('_id user_id code gender points birth_date ').lean();
    }
    if (!existingChild) {
...
async function fetchTaskStatusByChildId(childId) {
  ...
  if (!existchild) {
    // match parent-side behavior: return zeros instead of null so client gets meaningful data
    return {
      pending: 0,
      completed: 0,
      expired: 0,
